...
def main():
	...
	for f in files:
		...
		#print("shape: " + str(polymerXYZs[currFile,:,:].shape))

		distance_mat=pdist(np.squeeze(polymerXYZs[currFile,:,:]))
		#print("shape: " + str(distance_mat.shape))

		medians_dist = np.median(distance_mat)

		distances[currFile] = medians_dist		
	...
	sortedDistances = np.flip(sortedDistances,axis=0)
