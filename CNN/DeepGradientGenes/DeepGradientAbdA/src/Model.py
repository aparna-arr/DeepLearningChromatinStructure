...
class ModelKeras():
	...
	def run(self):
		...
		tfopt = tf.contrib.opt.AdamWOptimizer(learning_rate = self.learning_rate, weight_decay = self.weight_decay)

		model.compile(optimizer=tfopt, loss=self.compute_cost_weighted, metrics = ['acc'])
		if not os.path.isdir(self.savedir):
			subprocess.run(['mkdir', '-p', self.savedir])

		model.fit(self.X_train, self.Y_train, epochs=self.num_epochs, batch_size=self.minibatch_size, callbacks=[ival,ivaltrain])
...
class RestoreModelKeras():
	...
	def return_model(self):
		return load_model(self.model_loc)	
	def run(self, X_test, Y_test):
		ops.reset_default_graph()
