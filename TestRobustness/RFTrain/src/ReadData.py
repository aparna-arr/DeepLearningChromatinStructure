...
class ReadData():	
	def __init__(self, X_train_file, Y_train_file, X_dev_file, Y_dev_file, return_avg_std_training=False, return_matrix=False, normalize=True):
	...
	def load_data_rf(self):
		if self.normalize == False:
			trainX_orig = self.load_xyz(self.X_train_file, prev_mean_exist=False, normalize=self.normalize)	
			devX_orig = self.load_xyz(self.X_dev_file, prev_mean_exist=False, normalize=self.normalize)	
		else:
			trainX_orig, prev_mean, prev_std = self.load_xyz(self.X_train_file, prev_mean_exist=False, normalize=self.normalize)	
			devX_orig = self.load_xyz(self.X_dev_file, prev_mean, prev_std, prev_mean_exist=True, normalize=self.normalize)
		...
		trainX_reshaped = trainX_orig.reshape(trainX_orig.shape[0], -1)
		devX_reshaped = devX_orig.reshape(devX_orig.shape[0], -1)	

		Y_train = np.squeeze(trainY_orig)
		Y_dev = np.squeeze(devY_orig)
	...
	def load_data(self):
		trainX_orig, prev_mean, prev_std = self.load_xyz(self.X_train_file, prev_mean_exist=False)	
		devX_orig = self.load_xyz(self.X_dev_file, prev_mean, prev_std, prev_mean_exist=True)
	...
	def load_xyz(self, xyzfile,prev_mean=None, prev_std=None, prev_mean_exist=False, normalize=True):
