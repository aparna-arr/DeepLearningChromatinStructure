...
class ModelDriverInterpretDeepGradient():
	...
	def init_interpret(self):
		self.interpretObj = InterpretDeepGradient(self.data['X_dev'], self.data['Y_dev'], self.data['X_train'], self.train_mean, self.train_std, self.params['model_loc'], self.params['tag'], self.params['minibatch_size'], self.data['X_dev_nonorm'], self.params['threshold_sigmoid'])

	def run(self):
		self.interpretObj.run()

	def run_avg(self):
		self.interpretObj.run_avg()

	def print_sigmoids(self):
		self.interpretObj.print_sigmoids()
...
class ModelDriverInterpret():
	...
	def make_other_gene_plots(self):
		...
		fig = plt.figure()
		plt.title("ROC Curve")
		plt.plot(fprTrain, tprTrain, 'navy', lw = 2, label = 'Ubx Train AUC = %0.2f' % aucTrain)
		plt.plot(fprTest, tprTest, 'red', lw=2, label = 'abdA AUC = %0.2f' % aucTest)
		...
		fig.savefig("abdA_ROC_curve.pdf", bbox_inches='tight')
	...
	def make_plots(self):
		...
		# print out test_y_pred
		outpred = open("Y_pred.txt", "w")
		outpred.write("\n".join([str(int(x[0])) for x in test_y_pred]))
		outpred.close()
		...
		plt.imshow(stdev_pos[:,:,0], cmap = cmap, vmin=0.8,vmax=1.2)
		plt.colorbar()
		...
		plt.imshow(stdev_neg[:,:,0], cmap = cmap, vmin=0.8,vmax=1.2)
		plt.colorbar()
...
class ModelDriver():
	...
	def init_model(self, percTrain = 1.0):
		if percTrain < 1.0:
			m_train = math.floor(percTrain * self.data['X_train'].shape[0])
			np.random.seed(1)
			randomIdxs = np.arange(m_train)
			np.random.shuffle(randomIdxs)
			trainIdxs = randomIdxs[:m_train]
			
			self.data['X_train'] = self.data['X_train'][trainIdxs]
			self.data['Y_train'] = self.data['Y_train'][trainIdxs]

			print("Shape of X_train and Y_train after downsampling to " + str(percTrain) + " proportion of original dataset")
			print("X_train: " + str(self.data['X_train'].shape))
			print("Y_train: " + str(self.data['Y_train'].shape))
