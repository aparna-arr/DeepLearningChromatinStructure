...
class InterpretDeepGradient:
	...
	def print_sigmoids(self):
		...
		print("dumping sigmoids to .mat")
		pos_idx_bool = (self.Y_test > 0)
		neg_idx_bool = (self.Y_test < 1)

		sigmoid_Z_pos = sigmoid_Z[np.squeeze(pos_idx_bool)]
		sigmoid_Z_neg = sigmoid_Z[np.squeeze(neg_idx_bool)]

		print("dumping sigmoids to .mat")
		import scipy.io
		scipy.io.savemat(self.tag+'_ON_sigmoid_values.mat', {'on_sigmoids' : sigmoid_Z_pos})
		scipy.io.savemat(self.tag+'_OFF_sigmoid_values.mat', {'off_sigmoids' : sigmoid_Z_neg})
	...
	def run_avg(self):
		...
		print("dumping sigmoids to .mat")
		import scipy.io
		scipy.io.savemat(self.tag+'_ON_sigmoid_values.mat', {'on_sigmoids' : pos_shap})
		scipy.io.savemat(self.tag+'_OFF_sigmoid_values.mat', {'off_sigmoids' : neg_shap})
	...
	def most_neg_shap(self,shap_values, test_images):
		...
		f = plt.figure()
		plt.imshow(minDist, cmap = 'Greys_r')
		plt.colorbar()
	...
	def most_pos_shap(self,shap_values, test_images):
		...
		f = plt.figure()
		plt.imshow(maxDist, cmap = 'Greys_r')
		plt.colorbar()
